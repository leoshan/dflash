#!/bin/bash
# DFlash 测试脚本 (Qwen3.5-9B on SGLang)
...
export LD_LIBRARY_PATH="$PROJECT_ROOT/.venv-sglang/lib/python3.12/site-packages/nvidia/cuda_runtime/lib:$LD_LIBRARY_PATH"

# 1. 启动 SGLang 服务 (DFlash)
echo "正在启动 SGLang 服务 (DFlash, 9B)..."
"$PYTHON_BIN" -m sglang.launch_server \
    ...
    --port 30000 \
    --trust-remote-code > sglang_dflash.log 2>&1 &
...
while true; do
    if curl -s http://127.0.0.1:30000/v1/models > /dev/null; then
        echo "SGLang (DFlash) 服务已就绪！"
        break
    fi
    if ! kill -0 $SGL_PID 2>/dev/null; then
        echo "错误: SGLang 服务启动失败。请检查 sglang_dflash.log。"
        exit 1
    fi
    count=$((count + 1))
...
"$PYTHON_BIN" -m dflash.benchmark \
    ...
    --dataset gsm8k \
    --num-prompts 128 \
    --concurrency 1 > benchmark_dflash_raw.log 2>&1

# 4. 提取结果并保存
echo "测试完成，正在整理结果..."
cat benchmark_dflash_raw.log
